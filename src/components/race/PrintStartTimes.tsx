...
function generatePrintHTML(race: Race): string {
  ...
  const createTableHTML = (distance: '5km' | '10km', runners: Runner[]): string => {
    ...
    const tableRows = runners.map(runner => `<tr><td style="border: 1px solid #000; padding: 4px 6px; font-size: 10pt;">${runner.member_number}</td><td style="border: 1px solid #000; padding: 4px 6px; font-size: 10pt;">${runner.full_name}</td><td style="border: 1px solid #000; padding: 4px 6px; font-size: 10pt;">${getHandicap(runner)}</td><td style="border: 1px solid #000; padding: 4px 6px; font-size: 10pt;">${getChampionshipPoints(runner)}</td><td style="border: 1px solid #000; padding: 4px 6px; font-size: 10pt;">${formatStartTime(runner)}</td></tr>`).join('')

    return `<div style="margin-bottom: 12px;"><h2 style="font-size: 14px; font-weight: bold; margin: 0 0 2px 0;">${race.name} - ${distance} Start Times</h2><p style="font-size: 11px; color: #666; margin: 0 0 6px 0;">Date: ${race.date}</p><table style="width: 100%; border-collapse: collapse; font-size: 10pt;"><thead><tr><th style="background-color: #f3f4f6; border: 1px solid #000; padding: 3px 5px; text-align: left; font-weight: bold; font-size: 9pt;">Member #</th><th style="background-color: #f3f4f6; border: 1px solid #000; padding: 3px 5px; text-align: left; font-weight: bold; font-size: 9pt;">Name</th><th style="background-color: #f3f4f6; border: 1px solid #000; padding: 3px 5px; text-align: left; font-weight: bold; font-size: 9pt;">Handicap</th><th style="background-color: #f3f4f6; border: 1px solid #000; padding: 3px 5px; text-align: left; font-weight: bold; font-size: 9pt;">Champ Points</th><th style="background-color: #f3f4f6; border: 1px solid #000; padding: 3px 5px; text-align: left; font-weight: bold; font-size: 9pt;">Start Time</th></tr></thead><tbody>${tableRows}</tbody></table></div>`
  }

  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${race.name} - Start Times</title><style>* { margin: 0; padding: 0; } @page { margin: 0.3cm; size: A4 portrait; } html { margin: 0; padding: 0; } body { font-family: Arial, sans-serif; margin: 0.3cm; padding: 0; color: #000; line-height: 1.15; } h2 { margin: 0 0 1px 0; padding: 0; } p { margin: 0 0 4px 0; padding: 0; } div { margin: 0; padding: 0; } table { page-break-inside: avoid; margin: 0; padding: 0; border-spacing: 0; } tr { page-break-inside: avoid; } .section { page-break-after: avoid; margin-bottom: 8px; padding: 0; }</style></head><body>${createTableHTML('5km', runners5k)}<div style="page-break-after: always; height: 0; margin: 0; padding: 0;"></div>${createTableHTML('10km', runners10k)}</body></html>`
}
